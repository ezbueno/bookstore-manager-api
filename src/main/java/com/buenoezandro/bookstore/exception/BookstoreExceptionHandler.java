package com.buenoezandro.bookstore.exception;
...
public class BookstoreExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException exception,
		...
		exception.getBindingResult().getFieldErrors().forEach(fieldErrors -> errors
				.add("Field: " + fieldErrors.getField().toUpperCase() + " " + fieldErrors.getDefaultMessage()));
		...
		return this.buildResponseEntity(HttpStatus.BAD_REQUEST, "Informed argument(s) validation error(s)", errors);
	}
	...
	private ResponseEntity<Object> buildResponseEntity(HttpStatus httpStatus, String message, List<String> errors) {
		var apiError = ApiError.builder().code(httpStatus.value()).status(httpStatus.getReasonPhrase()).message(message)
